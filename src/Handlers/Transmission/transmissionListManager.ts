...
export const addGroupToTransmission = (chatId: string, boundary: string, list: string = 'default') => {
...
export const getTransmissionList = (boundary: string, list: string = 'default') => {
    return TransmissionListDb.getAllEntities().filter(group => group.list === list && boundary === group.boundary);
};
